...
fn test_zero_drift_zero_thrust() {
    ...
    let ideal_pos = Vector3 { x: 6871000.0, y: 0.0, z: 0.0 };
    let ideal_vel = Vector3 { x: 0.0, y: 7600.0, z: 0.0 };
    ...
    let magnitude = (maneuver.delta_v.x.powi(2) + maneuver.delta_v.y.powi(2) + maneuver.delta_v.z.powi(2)).sqrt();
...
fn test_directional_correction() {
    ...
    let current_pos = Vector3 { x: 6872000.0, y: 0.0, z: 0.0 }; // 1000m off-target
    let reference_pos = Vector3 { x: 6871000.0, y: 0.0, z: 0.0 };
    let current_vel = Vector3 { x: 0.0, y: 7600.0, z: 0.0 };
    let config = RecoveryConfig::default();

    let maneuver = compute_recovery_maneuver(
        &current_pos, &current_vel, 
        &reference_pos, &current_vel, 
        &config
    );
...
fn test_fuel_clamping_safety() {
    ...
    let current_pos = Vector3 { x: 9999999.0, y: 9999999.0, z: 0.0 }; // Drastically lost
    let reference_pos = Vector3 { x: 6871000.0, y: 0.0, z: 0.0 };
    let current_vel = Vector3 { x: 0.0, y: 7600.0, z: 0.0 };
    let config = RecoveryConfig::default();
